...
if (!empty($_GET['id_theme'])) {
    ...
    header("location:/PAGE/cartes.php");
}
...
if (isset($_POST["ajouter"])) {
    ...
    if (!empty($_POST["questionRecto"]) && !empty($_POST["questionVerso"])) {
        ...
        $sql = "INSERT INTO carte ( id_theme, recto, verso, date_creation, date_modification, img_recto, img_verso) 
        VALUES (:id_theme, :recto, :verso, :date_creation, :date_modification, :img_recto, :img_verso)";
        ...
        if ($_FILES["imgRecto"]["size"] != 0) {
            ...
            $requete ->bindParam(':id_carte', $lastId); 
            $requete->execute();
        }
        ...
        header("location: /PAGE/cartes.php?id_theme=" . $id_theme);
    }
} else {
    header("location: /PAGE/cartes.php?id_theme=" . $id_theme);
